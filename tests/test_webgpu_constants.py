...
def test_stride_case_insensitive():
...
def test_vertex_format_conversions():
...
def test_vertex_format_case_insensitive():
...
def test_vertex_format_invalid_type():
    """Test that vertex_format raises KeyError for invalid types."""
    with pytest.raises(KeyError):
        NGLToWebGPU.vertex_format("invalid")

    with pytest.raises(KeyError):
        NGLToWebGPU.vertex_format("mat2")  # Only vectors supported for vertex format


def test_stride_values_are_positive():
...
def test_vertex_format_strings():
...
def test_matrix_stride_consistency():
