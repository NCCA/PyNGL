...
def test_rgb_widget_initial_value(qtbot):
    ...
    assert w.colour() == Vec3(1.0, 1.0, 1.0)
...
def test_rgb_widget_constructor_initial_value_and_name(qtbot):
...
def test_property_accessors(qtbot):
    ...
    w.value = Vec3(0.1, 0.2, 0.3)
    assert w.colour() == Vec3(0.1, 0.2, 0.3)
...
def test_set_colour_blocks_channel_signals_but_emits_colourChanged(qtbot):
    ...
    assert gs == []
    assert bs == []
    # combined colourChanged emitted with Vec3
    emitted = sig.args[0]
...
def test_show_color_dialog_applies_selection_and_emits(monkeypatch, qtbot):
    ...
    def fake_get_color(current, parent, title):
        return QColor.fromRgbF(0.2, 0.3, 0.4)
    ...
    emitted = sig.args[0]
    assert emitted.x == pytest.approx(0.2, rel=1e-4)
    assert emitted.y == pytest.approx(0.3, rel=1e-4)
    assert emitted.z == pytest.approx(0.4, rel=1e-4)
