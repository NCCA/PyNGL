...
### [`abstract_vao.py`](src/ncca/ngl/opengl/abstract_vao.py)
*   [`VertexData`](src/ncca/ngl/opengl/abstract_vao.py): A simple data structure to hold vertex data for a VAO. It stores the data as a numpy array, the size of the data, and the drawing mode.
*   [`AbstractVAO`](src/ncca/ngl/opengl/abstract_vao.py): An abstract base class for Vertex Array Objects (VAOs). It defines the interface for different VAO implementations, including methods for binding, drawing, setting data, and managing the VAO's lifecycle.

### [`base_mesh.py`](src/ncca/ngl/opengl/base_mesh.py)
*   [`Face`](src/ncca/ngl/opengl/base_mesh.py): A simple data structure for a mesh face, holding indices for vertices, UVs, and normals.
*   [`BaseMesh`](src/ncca/ngl/opengl/base_mesh.py): A base class for mesh geometry. It provides storage for vertices, normals, UVs, and faces, and includes methods for creating a VAO from the mesh data, calculating dimensions, and drawing the mesh.
...
### [`multi_buffer_vao.py`](src/ncca/ngl/opengl/multi_buffer_vao.py)
*   [`MultiBufferVAO`](src/ncca/ngl/opengl/multi_buffer_vao.py): A VAO implementation that can manage multiple vertex buffers. This is useful for separating different types of vertex attributes (e.g., positions, colors, normals) into different buffers.
...
### [`primitives.py`](src/ncca/ngl/opengl/primitives.py)
*   [`Primitives`](src/ncca/ngl/opengl/primitives.py): A static class for creating and drawing pre-defined geometric primitives. It uses `PrimData` to generate the vertex data and `VAOFactory` to create VAOs for rendering.

### [`pyside_event_handling_mixin.py`](src/ncca/ngl/opengl/pyside_event_handling_mixin.py)
*   [`PySideEventHandlingMixin`](src/ncca/ngl/opengl/pyside_event_handling_mixin.py): A mixin class for PySide6 applications that provides common event handling for mouse-based camera control (rotation, translation, zoom) and keyboard shortcuts.
...
### [`shader.py`](src/ncca/ngl/opengl/shader.py)
*   [`Shader`](src/ncca/ngl/opengl/shader.py): Represents a single OpenGL shader object (e.g., vertex, fragment). It handles loading source from a file, compiling the shader, and checking for errors.

### [`shader_lib.py`](src/ncca/ngl/opengl/shader_lib.py)
*   [`ShaderLib`](src/ncca/ngl/opengl/shader_lib.py): A singleton class that manages a library of shader programs. It provides a global point of access for loading, compiling, linking, and using shaders, as well as for setting uniform variables.

### [`shader_program.py`](src/ncca/ngl/opengl/shader_program.py)
*   [`ShaderProgram`](src/ncca/ngl/opengl/shader_program.py): A wrapper for an OpenGL shader program. It manages attaching shaders, linking the program, and provides an interface for setting uniform variables.

### [`simple_index_vao.py`](src/ncca/ngl/opengl/simple_index_vao.py)
*   [`SimpleIndexVAO`](src/ncca/ngl/opengl/simple_index_vao.py): A VAO implementation that uses an index buffer for indexed drawing. This is more efficient for meshes where vertices are shared between multiple faces.

### [`simple_vao.py`](src/ncca/ngl/opengl/simple_vao.py)
*   [`SimpleVAO`](src/ncca/ngl/opengl/simple_vao.py): A basic VAO implementation that uses a single buffer for non-indexed drawing.

### [`text.py`](src/ncca/ngl/opengl/text.py)
*   [`Text`](src/ncca/ngl/opengl/text.py): A class for rendering text in OpenGL. It uses `freetype-py` to create a texture atlas of font glyphs and renders text using a geometry shader to create quads for each character.

### [`texture.py`](src/ncca/ngl/opengl/texture.py)
*   [`Texture`](src/ncca/ngl/opengl/texture.py): A class for loading image files and creating OpenGL textures from them.
...
### [`vao_factory.py`](src/ncca/ngl/opengl/vao_factory.py)
*   [`VAOFactory`](src/ncca/ngl/opengl/vao_factory.py): A factory class for creating different types of VAOs. It allows for registering custom VAO creators and creating VAO instances by name.
